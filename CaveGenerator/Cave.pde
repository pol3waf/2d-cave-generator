public class Cave
{
      ...
      )
  {
    ...
    drawArrayData(grid, primitiveType);
  }
  ...
  **/
   ...
   */
  private void drawArrayData(int[][] grid, String primitive)
  {
    ...
    for (int i=0; i<sizeX; i++)
    {
      ...
      {
        fill(grid[i][j]);
        if (primitive.equals("rect"))
        {
          rect(i*cellX, j*cellY, (i+1)*cellX, (j+1)*cellY);
        }
        else if (primitive.equals("circle"))
        {
          float midx = ( (i * cellX) + ((i+1) * cellX) ) / 2;
          float midy = ( (j * cellY) + ((j+1) * cellY) ) / 2;
          float radiusX = cellX * 1;
          float radiusY = cellY * 1;
          ellipse(midx, midy, radiusX, radiusY);
        }
      }
    }
  }
